...
- (void)cueVideoById:(NSString *)videoId
    ...
    [self .delegate playerView:self didChangeToState:kYTPlayerStateStarted];
}
...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams {
    ...
    //NSNumber *width =  [NSNumber numberWithFloat:CGRectGetWidth(self.frame)];
    //NSNumber *height = [NSNumber numberWithFloat:CGRectGetHeight(self.frame)];
    //[playerParams setValue:width forKey:@"width"];
    //[playerParams setValue:height forKey:@"height"];
    ...
    //NSLog(@"[DEBUG] <YTPlayerView> %@", embedHTML);
